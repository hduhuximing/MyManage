...
public class LoginController {
    ...
    public FebsResponse userOnline(String username) throws Exception {
        ...
        Set<String> userOnlineStringSet =
                redisService.zrangeByScore(ComConstant.ACTIVE_USERS_ZSET_PREFIX, now, "+inf");
    ...
    public void kickout(@NotBlank(message = "{required}") @PathVariable String id) throws Exception {
        ...
        if (kickoutUser != null && StringUtils.isNotBlank(kickoutUserString)) {
            // 删除 zset中的记录
            redisService.zrem(ComConstant.ACTIVE_USERS_ZSET_PREFIX, kickoutUserString);
            // 删除对应的 token缓存
            redisService.del(ComConstant.TOKEN_CACHE_PREFIX + kickoutUser.getToken() + "." + kickoutUser.getIp());
        }
    }
    ...
    private String saveTokenToRedis(User user, JWTToken token, HttpServletRequest request) throws Exception {
        ...
        this.redisService.zadd(ComConstant.ACTIVE_USERS_ZSET_PREFIX,
                Double.valueOf(token.getExipreAt()), mapper.writeValueAsString(activeUser));
        // redis 中存储这个加密 token，key = 前缀 + 加密 token + .ip
        this.redisService.set(ComConstant.TOKEN_CACHE_PREFIX + token.getToken() + StringPool.DOT + ip,
