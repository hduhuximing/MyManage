...
public class MovieController {
    ...
    public FebsResponse getMoiveHot() throws FebsException {
        try {
            String data = HttpUtil.sendSSLPost(ComConstant.TIME_MOVIE_HOT_URL, "locationId=328");
    ...
    public FebsResponse getMovieComing() throws FebsException {
        try {
            String data = HttpUtil.sendSSLPost(ComConstant.TIME_MOVIE_COMING_URL, "locationId=328");
            return new FebsResponse().data(data);
        } catch (Exception e) {
    ...
    public FebsResponse getDetail(@NotBlank(message = "{required}") String id) throws FebsException {
        try {
            String data = HttpUtil.sendSSLPost(ComConstant.TIME_MOVIE_DETAIL_URL, "locationId=328&movieId=" + id);
            return new FebsResponse().data(data);
        } catch (Exception e) {
            message = "获取影片详情失败";
            log.error(message, e);
    ...
    public FebsResponse getComments(@NotBlank(message = "{required}") String id) throws FebsException {
        try {
            String data = HttpUtil.sendSSLPost(ComConstant.TIME_MOVIE_COMMENTS_URL, "movieId=" + id);
            return new FebsResponse().data(data);
        } catch (Exception e) {
            message = "获取影片评论失败";
            log.error(message, e);
